...
function approachErrorMessage(error: unknown): string {
  const body = error as { status?: number; error?: string };
  if (body.status === 409) return "This scan changed since you opened it. Reload, then check again.";
  if (body.status === 404) return body.error ?? "That object is not here anymore.";
  return typeof body.error === "string" ? body.error : "The approach check failed. Try again in a moment.";
}

function ApproachReportDetails({ report }: { report: ApproachReport }) {
  const status = STATUS_STYLE[report.status] ?? STATUS_STYLE.needs_verification;
  return (
 ...
 */
export function ApproachCheck({ nodeId, run }: {
  nodeId: string;
  ...
  const check = useCallback(async () => {
    ...
    try {
      ...
      setProblem(approachErrorMessage(error));
    } finally {
      setChecking(false);
    }
  }, [nodeId, run]);
  ...
  return (
    <div className="flex flex-col gap-2 border-t border-rule/60 pt-2">
      ...
      {report && <ApproachReportDetails report={report} />}
    </div>
  );
}
