import { BackSide, BufferAttribute, BufferGeometry, DataTexture, Mesh, MeshBasicMaterial, MeshStandardMaterial, RGBAFormat, UnsignedByteType } from "three";
...
describe("painted scan materials", () => {
  ...
  it("uses an emissive photograph when it is the unlit source and preserves material groups", () => {
    ...
    expect((group[0] as MeshBasicMaterial).map).toBe(emissiveMap);
  ...
  it("does not enable vertex colors for a UV-only photogrammetry primitive", () => {
    ...
    expect(painted.map).toBe(mesh.material.emissiveMap);
  });
});
